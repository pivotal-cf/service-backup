...
var _ = Describe("Parsing properties", func() {
	Context("Valid backup config with alerts configured", func() {
		var (
			cron     string
			executor Executor
			alerts   *ServiceAlertsClient
		)
		BeforeEach(func() {
			executor, cron, alerts, _ = Parse([]string{"cmd", "fixtures/valid_backup_with_alerts.yml"})
		})
	...
	Context("Valid backup config without alerts configured", func() {
		var alerts *ServiceAlertsClient

		BeforeEach(func() {
			_, _, alerts, _ = Parse([]string{"cmd", "fixtures/valid_backup_without_alerts.yml"})
		})
