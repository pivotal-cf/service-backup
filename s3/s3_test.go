...
var _ = Describe("regionFromEndpoint", func() {
	DescribeTable("extracts region from standard AWS S3 endpoint URLs",
		func(endpointURL, expectedRegion string) {
			Expect(s3.RegionFromEndpoint(endpointURL)).To(Equal(expectedRegion))
		},
		// newer path-style format
		Entry("s3.REGION.amazonaws.com", "https://s3.us-west-2.amazonaws.com", "us-west-2"),
		Entry("s3.REGION.amazonaws.com eu", "https://s3.eu-west-1.amazonaws.com", "eu-west-1"),
		Entry("s3.REGION.amazonaws.com ap", "https://s3.ap-southeast-1.amazonaws.com", "ap-southeast-1"),
		// legacy hyphenated format
		Entry("s3-REGION.amazonaws.com", "https://s3-us-west-2.amazonaws.com", "us-west-2"),
		Entry("s3-REGION.amazonaws.com eu", "https://s3-eu-central-1.amazonaws.com", "eu-central-1"),
		// global endpoint → us-east-1
		Entry("global endpoint", "https://s3.amazonaws.com", "us-east-1"),
		// custom / non-AWS
		Entry("custom non-AWS endpoint", "https://s3h.uat.dbs.com", ""),
		Entry("localhost", "http://localhost:9000", ""),
		Entry("empty string", "", ""),
	)
})
	...
	Describe("CreateS3Client", func() {
		...
		Context("when a custom endpoint is configured", func() {
			It("uses path-style addressing so the bucket name is in the URL path", func() {
				client, err := s3.CreateS3Client(logger, "access", "secret", server.URL, "us-east-1")
				Expect(err).NotTo(HaveOccurred())

				s3Client := s3.New("name", "", server.URL, "us-east-1", "access", "secret", "", upload.RemotePathFunc("test-bucket", ""))
			...
			It("does not send extra payload checksum headers (x-amz-checksum-*) that non-AWS S3 endpoints reject", func() {
				...
				s3Client := s3.New("name", "", server.URL, "us-east-1", "access", "secret", "", upload.RemotePathFunc("", ""))
		...
		Context("when no custom endpoint is configured (standard AWS)", func() {
			It("creates a client without error", func() {
				client, err := s3.CreateS3Client(logger, "access", "secret", "", "us-east-1")
				Expect(err).NotTo(HaveOccurred())
