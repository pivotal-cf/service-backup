...
// upload) is reverse-proxied through to a real S3-compatible backend so
...
var _ = Describe("strict checksum gateway (TNZ-127170 repro against a real S3-compatible backend)", func() {
	...
	BeforeEach(func() {
		...
		setupClient := s3.New("setup", "", gateway.URL, "us-east-1", accessKey, secretKey, "", true, "sha256", upload.RemotePathFunc(bucket, ""))
	...
	Context("single-part upload (small file, PutObject)", func() {
		...
		It("fails with the reported error when no checksum_algorithm is configured (today's default)", func() {
			client, err := s3.CreateS3Client(logger, accessKey, secretKey, gateway.URL, "us-east-1", true)
			Expect(err).NotTo(HaveOccurred())

			s3Client := s3.New("name", "", gateway.URL, "us-east-1", accessKey, secretKey, "", true, "", upload.RemotePathFunc("", ""))
			err = s3Client.UploadFile(logger, client, tmpFilePath, bucket+"/small-none.txt")

			Expect(err).To(MatchError(ContainSubstring("InvalidRequest: The checksum algorithm must SHA256")))
		})

		It("succeeds when checksum_algorithm is set to sha256", func() {
			client, err := s3.CreateS3Client(logger, accessKey, secretKey, gateway.URL, "us-east-1", true)
			Expect(err).NotTo(HaveOccurred())

			s3Client := s3.New("name", "", gateway.URL, "us-east-1", accessKey, secretKey, "", true, "sha256", upload.RemotePathFunc("", ""))
			err = s3Client.UploadFile(logger, client, tmpFilePath, bucket+"/small-sha256.txt")
	...
	Context("multipart upload (large file, CreateMultipartUpload + UploadPart)", func() {
		...
		It("fails with the reported error when no checksum_algorithm is configured (today's default)", func() {
			...
			s3Client := s3.New("name", "", gateway.URL, "us-east-1", accessKey, secretKey, "", true, "", upload.RemotePathFunc("", ""))
			err = s3Client.UploadFile(logger, client, tmpFilePath, bucket+"/large-none.txt")
		...
		It("succeeds when checksum_algorithm is set to sha256", func() {
			...
			s3Client := s3.New("name", "", gateway.URL, "us-east-1", accessKey, secretKey, "", true, "sha256", upload.RemotePathFunc("", ""))
			err = s3Client.UploadFile(logger, client, tmpFilePath, bucket+"/large-sha256.txt")
